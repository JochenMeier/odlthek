...
var Mailer = {
  ...
  sendBookingHandoutMail: function(data){
    async.parallel({
      user: function(callback){
        UserModel.findById(data.userId, callback);
      },
      ...
      admin: function(callback){
        UserModel.findById(data.handoutUser, callback);
      } 
    }, function (err, p){
      var msg ='';
      msg += 'Hallo ' + p.user.name + ' ,\n';
      msg += '\n';
      msg += p.admin.name + ' (' + p.admin.email + ') hat so eben dein gewünschtes Device herausgegeben:\n';
      msg += 'Gerät: #' + p.gadget.hwid + ' (' + p.gadget.name + ')\n';
      msg += 'Start: ' + moment(data.booking.start).format(process.env.DATE_TIME_FORMAT) + '\n';
      msg += 'End: ' + moment(data.booking.end).format(process.env.DATE_TIME_FORMAT) + '\n';

     sendMail('Device herausgegeben', msg);
    });
  },


  sendBookingTakebackMail: function(data){
    async.parallel({
      user: function(callback){
        UserModel.findById(data.userId, callback);
      },
      ...
      admin: function(callback){
        UserModel.findById(data.takebackUser, callback);
      }
    }, function (err, p){
      var msg ='';
      msg += 'Hallo ' + p.user.name + ' ,\n';
      msg += '\n';
      msg += p.admin.name + ' (' + p.admin.email + ') hat so eben dein geliehenes Device in Empfang genommen:\n';
      msg += 'Gerät: #' + p.gadget.hwid + ' (' + p.gadget.name + ')\n';
      msg += 'Start: ' + moment(data.booking.start).format(process.env.DATE_TIME_FORMAT) + '\n';
      msg += 'End: ' + moment(data.booking.end).format(process.env.DATE_TIME_FORMAT) + '\n';

      sendMail('Device zurückgenommen', msg);
    });
  },
  ...
  sendBookingExpiredMail: function (data, callback) {
    async.parallel({
      ...
      sendMail('Rückgabedatum überschritten ', msg, null, callback(err));
    });
  },
